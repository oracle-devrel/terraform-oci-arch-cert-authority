...
resource "oci_certificates_management_certificate_authority" "test_certificate_authority" {
  ...
  kms_key_id     = var.create_new_kms_vault ? oci_kms_key.vault_key[0].id : var.kms_key_ocid
  name           = "${var.certificate_authority_name}-${local.random_id}"
  ...
  certificate_revocation_list_details {
    ...
    object_storage_config {
      #Required
      object_storage_bucket_name        = oci_objectstorage_bucket.vault_bucket.name
      ...
      #Optional
      object_storage_namespace = oci_objectstorage_bucket.vault_bucket.namespace
    }
